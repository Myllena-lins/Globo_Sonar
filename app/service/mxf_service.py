...
class MXFService:
    ...
    async def create_mxf_record(self, db: AsyncSession, file_name: str, file_path: str):
        mxf = await self.repository.save_file_record(db, file_name, file_path)
        return mxf
    ...
    async def get_mxf_details(self, db: AsyncSession, mxf_id: int):
        """
        Retorna o MXF com todas as faixas de áudio e seus time ranges.
        """
        mxf = await self.repository.get_mxf_with_tracks(db, mxf_id)
        return mxf
